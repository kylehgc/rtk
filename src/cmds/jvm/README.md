...
- **Maven Daemon (`rtk mvnd`)** — `mvnd` is a separate entry point (`run_daemon`), not a `mvn` alias: it shares phase detection and every filter, but always executes `mvnd` and is never substituted by a `./mvnw` wrapper found in the working directory. The daemon's rolling console UI only engages on a TTY, so what rtk captures is ordinary Maven log output and the existing filters apply unchanged. Two known limits, neither a regression — both already apply to `mvn -T`, and both need a real captured `mvnd` fixture before they can be fixed honestly:
  - Daemon status chatter (`Connecting to the daemon`, …) carries no `[INFO]`/`[ERROR]` prefix, so the keep-list filters (`filter_surefire`, `filter_compile`, `filter_package`) drop it — but `filter_quiet` has no keep-list, only a deliberate "keep anything else" safety net, so `rtk mvnd -q` passes that chatter through where `rtk mvn -q` emits nothing.
  - `mvnd` builds multi-module reactors in parallel by default. Interleaved `[INFO] Running …` lines from concurrent modules make `SurefireBlock` flush open blocks verbatim, so savings degrade toward passthrough on large reactors. Degrades gracefully (keeps signal, loses compression) per the fallback rule.
