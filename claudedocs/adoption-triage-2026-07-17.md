...
**Verification sweep 2026-07-17** (see section below): every rank re-verified against current develop before adoption. Ranks 2 and 8 turned out already fixed upstream by other merged work — struck out, tickets to close.

| Rank | Adopt | Why |
|---|---|---|
| 1 | ✅ ADOPTED [#3031](https://github.com/rtk-ai/rtk/pull/3031) hook: emit ask decision for Claude rewrites | Adopted as fork PR [kylehgc/rtk#10](https://github.com/kylehgc/rtk/pull/10) (issue #2), merged 2026-07-17 |
| 2 | ~~[#2263](https://github.com/rtk-ai/rtk/pull/2263) permissions: newline-separated commands bypass allow-rules~~ SUPERSEDED | Already fixed on develop by upstream `952245d` (2026-06-03): `split_for_permissions` splits on `\n`/`\r`/`&`/subshells. Verified by live repro — `git log\ngit push --force-with-lease` gets `ask`, quoted newlines still `allow`. Close issue #3 |
| 3 | ✅ ADOPTED [#2997](https://github.com/rtk-ai/rtk/pull/2997) stream: lossy-decode instead of dropping output after non-UTF-8 byte | Adopted as fork PR [kylehgc/rtk#14](https://github.com/kylehgc/rtk/pull/14) (issue #4), merged 2026-07-17 |
| 4 | ✅ ADOPTED [#2717](https://github.com/rtk-ai/rtk/pull/2717) core: decode process output using Windows console code page | Adopted as fork PR [kylehgc/rtk#15](https://github.com/kylehgc/rtk/pull/15) (issue #5), merged 2026-07-17 |
| 5 | ✅ ADOPTED [#3029](https://github.com/rtk-ai/rtk/pull/3029) runner: surface failing tool's stderr under stdout-only filters | Adopted as fork PR [kylehgc/rtk#17](https://github.com/kylehgc/rtk/pull/17) (issue #6), merged 2026-07-17 |
| 6 | [#3028](https://github.com/rtk-ai/rtk/pull/3028) git: keep every commit in `git log --stat` | Fresh, verified 30/30; git filter is your daily driver (related: #2161). Re-verified by live repro on develop: 8 of 30 commits dropped, diffstats parsed as headers (git.rs:455/580 marker ordering); low conflict risk |
| 7 | vitest cluster: [#2982](https://github.com/rtk-ai/rtk/pull/2982) exit codes + [#1497](https://github.com/rtk-ai/rtk/pull/1497) shim recursion + [#1922](https://github.com/rtk-ai/rtk/pull/1922) metadata commands | Your problem area — batch as one adoption PR. All three verified still valid, BUT #2982/#1497 predate develop's vitest refactors and will NOT cherry-pick cleanly — plan a hand-port (re-implementation path per CONTEXT.md), order 1922 → 2982 → 1497; consider dropping #1497's synthetic-version node shim (its failing upstream CI check) |
| 8 | ~~[#937](https://github.com/rtk-ai/rtk/pull/937) npx: passthrough unknown packages instead of routing to npm_cmd~~ SUPERSEDED | Already fixed on develop via `npm_cmd::exec` passthrough (main.rs:2312, npm_cmd.rs:102) with regression test `test_npx_unknown_tool_passthrough` citing the same upstream bug (#815) — develop's fix is better (filtered + exit-code propagation vs raw passthrough). Close issue #9; optional: port #937's `test_npx_known_cmds_still_route` sanity test |

## Verification sweep — 2026-07-17

Lesson from rank 2: an upstream PR being OPEN does not mean its bug is unfixed — upstream sometimes merges a different fix for the same bug (952245d landed one day before #2263 was opened). **Repro-before-adopt is mandatory**, and staleness must be re-checked at adoption time, not triage time.

Method: for each first-batch entry, inspected the exact code the upstream PR patches on current develop, checked file history for superseding commits, and ran live repros against a develop-built binary where cheap. Results are folded into the table above. Net: 6 of 8 open tickets still valid (#1, #4, #5, #6, #7, #8 — with #8 needing hand-port), 2 superseded (#3, #9).

Replacement candidates promoted from runners-up (filed as new tickets): [#3027](https://github.com/rtk-ai/rtk/pull/3027) non-UTF-8 argv panic in raw-execution fallback (✅ ADOPTED as fork PR [kylehgc/rtk#18](https://github.com/kylehgc/rtk/pull/18), issue #12) · [#2965](https://github.com/rtk-ai/rtk/pull/2965) pipe-segment rewrites with shape-changing filters (✅ ADOPTED as fork PR [kylehgc/rtk#16](https://github.com/kylehgc/rtk/pull/16), issue #13). Both merged 2026-07-17.

**Runners-up** (next batch material — since filed as batch-2 tickets, see [adoption-batch2-2026-07-19.md](adoption-batch2-2026-07-19.md)): ~~[#3027](https://github.com/rtk-ai/rtk/pull/3027) non-UTF-8 argv panic~~ ADOPTED · ~~[#2965](https://github.com/rtk-ai/rtk/pull/2965) pipe-segment rewrites~~ ADOPTED · [#2475](https://github.com/rtk-ai/rtk/pull/2475) hook flag injection · [#2565](https://github.com/rtk-ai/rtk/pull/2565) hook stdin stall fail-open · [#2830](https://github.com/rtk-ai/rtk/pull/2830) proxy pipe hang (Windows-verified) · [#2952](https://github.com/rtk-ai/rtk/pull/2952) Windows discover drive-colon · [#2483](https://github.com/rtk-ai/rtk/pull/2483) hook tracking stats · [#1978](https://github.com/rtk-ai/rtk/pull/1978) gain absurd-figures cap · [#1951](https://github.com/rtk-ai/rtk/pull/1951) npm/pnpm rewrite coverage · [#2951](https://github.com/rtk-ai/rtk/pull/2951) git log -p passthrough.
