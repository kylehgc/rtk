...
| 3 | [#2997](https://github.com/rtk-ai/rtk/pull/2997) stream: lossy-decode instead of dropping output after non-UTF-8 byte | Silent output loss affecting 6 call sites; newer equivalent of #2049. Verified still present (stream.rs:303–464, 6× `map_while(Result::ok)`); low conflict risk |
| 4 | [#2717](https://github.com/rtk-ai/rtk/pull/2717) core: decode process output using Windows console code page | Mojibake on Windows — you will hit this; centralized decoder, 11/11 checks. Verified still absent on develop; adds `encoding_rs` dep; touches high-traffic files — adopt soon before drift |
| 5 | [#3029](https://github.com/rtk-ai/rtk/pull/3029) runner: surface failing tool's stderr under stdout-only filters | False-quiet failures in rtk err/test; failing-before tests. Verified still present (runner.rs:114–130 lacks the guard); low conflict risk. Bonus: makes src/cmds/README.md:97's stderr-passthrough claim true |
...
Replacement candidates promoted from runners-up (filed as new tickets): [#3027](https://github.com/rtk-ai/rtk/pull/3027) non-UTF-8 argv panic in raw-execution fallback · [#2965](https://github.com/rtk-ai/rtk/pull/2965) pipe-segment rewrites with shape-changing filters.
